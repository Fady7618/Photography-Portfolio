...
export default function LazyImage({
  ...
  if (hasError) {
    return (
      <div
        className={`bg-red-100 border-2 border-red-300 flex items-center justify-center ${masonry ? 'min-h-52' : 'aspect-4/3'} rounded-lg ${className}`}
      >
  ...
  if (masonry) {
    return (
      <div
        className={`relative overflow-hidden cursor-pointer group rounded-lg break-inside-avoid mb-6 ${className}`}
        onClick={onClick}
      >
        {isLoading && (
